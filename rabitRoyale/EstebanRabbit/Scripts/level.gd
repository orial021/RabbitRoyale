...
var PLAYER = preload("res://Scenes/menu/player_animate.tscn")
...
func _process(delta: float) -> void:
	path.set_process(path.get_progress() + 8 * delta)
...
func revive_player(player_name : String) -> void:
	...
	if player:
		...
		player.rotation_degress = Vector3.ZERO
		player.HEAD.rotation_degress = Vector3.ZERO
