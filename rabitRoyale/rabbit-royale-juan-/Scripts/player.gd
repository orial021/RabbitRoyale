...
func _input(event: InputEvent) -> void:
	...
	if Input.mouse_mode == Input.MOUSE_MODE_VISIBLE and event.is_action_pressed("ui_shot"):
		Input.mouse_mode = Input.MOUSE_MODE_CAPTURED
	if not is_dead:
		if is_on_floor() and event.is_action_pressed("ui_jump"):
...
func shot_ctrl() -> void:
	...
	bullet_instance.set_global_position($Head/Marker3D.get_global_position())
	bullet_instance.player_owner= self
	

@rpc("any_peer","call_local","reliable")
func damage_ctrl(attacker_name : String = ""):
	if not is_multiplayer_authority():
		return
	animations.hurt()
	lives -= 1
