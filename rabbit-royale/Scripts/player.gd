...
func shot_ctrl() -> void:
	...
	if $Head/RayCast3D.is_colliding():
		direction = $Head/RayCast3D.get_collision_point() - $Head/Marker1.global_position
	else:
		direction = $Head/Sprite3D.global_position - $Head/Marker1.global_position
	get_parent().add_child(bullet_instance, true)
	bullet.instance.set_multiplayer_authority(multiplayer.get_unique_id())
	bullet_instance.direction = direction
	bullet_instance.set_global_position($Head/Marker1.get_global_position())
	bullet_instance.player_owned = self
